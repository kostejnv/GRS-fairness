...
class SaeGroupRecommender(BaseGroupRecommender):
    def __init__(self, elsa: ELSA, sae: SAE, fusion_strategy: FusionStrategy, combine_features_strategy: CombineFeaturesStrategy, normalize_user_embedding: bool = False):
        self.elsa = elsa
        self.sae = sae
        self.fusion_strategy = fusion_strategy
        self.combine_features_strategy = combine_features_strategy
        self.normalize_user_embedding = normalize_user_embedding
    ...
    def recommend_for_group(self, group_input_interactions: torch.Tensor, k: Optional[int], mask: torch.Tensor) -> np.ndarray:          
        ...
        sparse_group_embedding = self.fusion_strategy.normalized_fuse(sparse_embedding, normalize_user_embedding=self.normalize_user_embedding)
        ...
        scores = torch.where(group_mask, -1000, scores)
        if k is None:
            k = scores.shape[-1]
