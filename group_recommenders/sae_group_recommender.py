...
class SaeGroupRecommender(BaseGroupRecommender):
    def __init__(self, elsa: ELSA, sae: SAE, fusion_strategy: FusionStrategy):
        self.elsa = elsa
        self.sae = sae
    ...
    def recommend_for_group(self, group_input_interactions: torch.Tensor, k: int | None, mask: torch.Tensor) -> np.ndarray:
        ...
        sparse_embedding, _, x_mean, x_std = self.sae.encode(dense_embedding)
        sparse_group_embedding = self.fusion_strategy.fuse(sparse_embedding)
